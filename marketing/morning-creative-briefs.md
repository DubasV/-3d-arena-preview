# Технические задания: утренние креативы

Статус: брифы для производства и раздельного теста. Это не креативы и не разрешение на загрузку, публикацию, включение кампании, изменение бюджетов или ставок.

Общие правила для всех брифов:

- Форматы: 1080×1080, 1080×1350, 1080×1920.
- CTA: `ЗАБРОНИРОВАТЬ`.
- Логотип: `assets/brand/mark.svg`, верхний левый угол, отступ 32 px при ширине 1080 px.
- Орнамент: только утверждённый исправленный паттерн.
- До внешнего действия требуется отдельное подтверждение владельца.

## Creative 1: Student — price

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-01.jpg
- Headline on image: 5 ЧАСОВ ОТ 540 ₽
- Support line: 08:00–13:00 · МОЛОДЁЖНАЯ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Нет первой пары? Проведите окно за игрой рядом с метро «Молодёжная».
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 2: Student — product

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-01.jpg
- Headline on image: 300 ГЦ ДО ПЕРВОЙ ПАРЫ
- Support line: 5 ЧАСОВ УТРОМ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Утренний пакет с 08:00 до 13:00: пять часов игры в зале «Киберспорт».
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 3: Shift — price

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-15.jpg
- Headline on image: ПОСЛЕ СМЕНЫ — ОТ 540 ₽
- Support line: 5 ЧАСОВ ИГРЫ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Утренний пакет с 08:00 до 13:00: пять часов игры от 540 ₽.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 4: Shift — product

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-15.jpg
- Headline on image: 300 ГЦ ИЛИ 2K
- Support line: УТРО В 3D АРЕНЕ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Выберите зал «Киберспорт» с 300 Гц или зал «Комфорт» с 2K.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним, неподтверждённые характеристики атмосферы

## Creative 5: Local — price

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-01.jpg
- Headline on image: 5 ЧАСОВ РЯДОМ С ДОМОМ
- Support line: ОТ 540 ₽ · МОЛОДЁЖНАЯ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Утренний пакет: пять часов игры от 540 ₽ рядом с метро «Молодёжная».
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 6: Local — product

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-15.jpg
- Headline on image: ДВА ЗАЛА НА ВЫБОР
- Support line: 300 ГЦ ИЛИ 2K
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: В клубе доступны залы «Киберспорт» с 300 Гц и «Комфорт» с 2K.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 7: Competitor — price

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-01.jpg
- Headline on image: УТРО: 5 ЧАСОВ ОТ 540 ₽
- Support line: М. МОЛОДЁЖНАЯ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Утренний пакет действует с 08:00 до 13:00. Пять часов игры от 540 ₽.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним, сравнения и обвинения конкурентов

## Creative 8: Competitor — product

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-01.jpg
- Headline on image: 300 ГЦ. 2K.
- Support line: ДВА ЗАЛА НА ВЫБОР
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Два зала: «Киберспорт» с 300 Гц и «Комфорт» с 2K.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним, сравнения и обвинения конкурентов, неподтверждённые характеристики атмосферы

## Creative 9: Returning — price

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-15.jpg
- Headline on image: ВЕРНИТЕСЬ УТРОМ
- Support line: 5 ЧАСОВ ОТ 540 ₽
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: Утренний пакет действует с 08:00 до 13:00. Пять часов игры от 540 ₽.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним

## Creative 10: Returning — product

- Formats: 1080×1080, 1080×1350, 1080×1920
- Source image: images/gallery/hall-12.jpg
- Headline on image: ПК ИЛИ PS5
- Support line: ВЫБЕРИТЕ СВОЙ ФОРМАТ
- CTA: ЗАБРОНИРОВАТЬ
- Logo: assets/brand/mark.svg, top-left, clear-space 32 px at 1080 px
- Ornament: only the approved corrected pattern
- Description: В клубе есть ПК-залы и зона PlayStation 5.
- Forbidden: «бесплатно», неподтверждённые рейтинги, энергетики, ночной доступ несовершеннолетним
